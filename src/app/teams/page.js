...
import { TeamData2k21,TeamData2k22, TeamData2k23, TeamData2k24 } from "@/app/lib/data/TeamData";
  ...
  return (
    ...
    <div className="min-h-screen w-full p-4 bg-[#0D1116] flex flex-col gap-28">
      ...
      <div>
        ...
        <div className={style.cardContainer}>
          {TeamData2k24.map((item) => {
            return (
              <TeamCard
                name={item.name}
                pos={item.pos}
                imgSrc={'/teams/2K24/'+item.imgSrc}
                lazyImgSrc={'/teams/2K24/lazy/'+item.imgSrc}
                socials={item.socials}
                key={item.name}
              />
            );
